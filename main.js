...
socket.on("load", function (data) {
    ...
    var newBudgies = JSON.parse(data.data);

    for (var i = 0; i < gameEngine.entities.length; i++) {
    	gameEngine.entities[i].removeFromWorld = true;
    }
    newBudgies.forEach(function (budgie) {
    	if (budgie !== null) {
    		var ent = new Circle(gameEngine);
	    	ent.x = budgie.x;
	    	ent.y = budgie.y;
	    	ent.food = budgie.food;
	    	ent.color = budgie.color;
	    	ent.velocity = { x: budgie.velocity.x, y: budgie.velocity.y };
